...
export default function JapanMap() {
  ...
  const onSelect = (state: string | null) => {
    ...
    const normalized = state
      ? state.replace(/[^\x00-\x7F]|[\s\u0000-\u001F\u007F-\u009F]/g, "").trim()
      : "";
    console.log("onSelect called with:", state, "→ normalized:", normalized);

    const prefName = prefMap[normalized];
    if (prefName) {
      setSelectedPref({ code: normalized, name: prefName });
      setModalOpen(true);
    } else {
        ...
        >
          ...
          >
            ...
            <div className={styles.modalBody}>
              <p>
                <strong>都道府県コード:</strong> {selectedPref.code}
              </p>
              <p>
                <strong>都道府県名:</strong> {selectedPref.name}
              </p>
